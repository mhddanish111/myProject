...
$sql="select p.id,p.imagepath,p.title,p.description,p.price,p.adddate,p.eurl as peurl,c.eurl as ceurl, s.eurl as seurl from product as p left join category as c on p.catid=c.id left join subcategory as s on s.catid=c.id where 1 $cond and p.status ='Y' and p.title Like '%$serchtext%' OR p.price Like '%$serchtext%' $orderby";	
...
<!-- welcome start//-->
<form action="<?php echo $_SERVER['REQUEST_URI'];?>" id="frmsearch" name="frmsearch" method="post" onsubmit="return pageSubmit('<?php echo $abspath; ?>');">
...
<div class="textfelid-searchbox">
      <label>
      <input name="textfield" type="text" id="textfield" value="<?php echo urldecode($_REQUEST['textfield']); ?>" class="inp5" />
      </label>
  </div>
  <div class="textfelid-searchbox1">
	...
	while($resultcategory = mysql_fetch_assoc($rscategory))
	{
		if($resultcategory['eurl']==$_REQUEST['selectcat'])
			echo'<option value ="'.$resultcategory['eurl'].'" selected="selected">'.stripslashes($resultcategory['catname']).'</option>';
		else
			echo'<option value ="'.$resultcategory['eurl'].'">'.stripslashes($resultcategory['catname']).'</option>';
	}
?>
  ...
  <option value="All-Subcategory">All Subcategory</option>
	...
	$sqlsub="select scat.id,scat.subcat,scat.eurl from subcategory as scat, category as cat where scat.catid=cat.id and scat.status='Y' and cat.eurl='".$_REQUEST['selectcat']."'";
	...
	if(mysql_num_rows($subsel))
	{
		while($resultsub = mysql_fetch_assoc($subsel))
		{
			...
			echo '<option value='.$resultsub['eurl'].' selected="selected">'.stripslashes($resultsub['subcat']).'</option>';
			else
			echo '<option value='.$resultsub['eurl'].'>'.stripslashes($resultsub['subcat']).'</option>';
		}
	} 
	?>
    </select>
		...
		if($rescat1['id']==$_REQUEST['selectcat'])
		{
			...
			while($ressub1 = mysql_fetch_assoc($rssub1))
			{
				if($_REQUEST['selectsub']==$ressub1['id'])
					echo'<div class="sublink"><a href="'.$abspath.''.strtolower($rescat1['eurl']).'/'.strtolower($ressub1['eurl']).'/12/1/1.html" id="sublink-active">'.stripslashes($ressub1['subcat']).'</a></div>';
				else
					echo'<div class="sublink"><a href="'.$abspath.''.strtolower($rescat1['eurl']).'/'.strtolower($ressub1['eurl']).'/12/1/1.html">'.stripslashes($ressub1['subcat']).'</a></div>';
			}
		}
  	}
?>
