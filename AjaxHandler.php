...
else if($_REQUEST["query"]=="selectsubsearch")
{
	...
	#select japanese Category Name
	//$subsel = mysql_query("select * from subcategory where status ='Y' and catid='$cat'");
	$sql="select scat.id,scat.subcat,scat.eurl from subcategory as scat, category as cat where scat.catid=cat.id and scat.status='Y' and cat.eurl='$cat'";
	...
	if(mysql_num_rows($subsel))
	{
		...
		while($result1 = mysql_fetch_assoc($subsel))
		{
			$responseText .= '<option value='.$result1['eurl'].'>'.stripslashes($result1['subcat']).'</option>';
		}
		$responseText .= '</select>';
	}
