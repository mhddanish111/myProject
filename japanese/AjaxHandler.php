...
//**************************************** Send Password************************
	...
	$UserMessage = '<table width="70%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#0033FF">
					...
					<tr>
					   <td align="left">Click <a href="http://47ddc.com/">Here</a> To Login,
					   </td>
					</tr>
					<tr><td>&nbsp;</td></tr>
					<tr><td>
...
else if($_REQUEST["query"]=="selectsub")
{
	...
	$subsel = mysql_query("select * from subcategory where status ='Y' and catid='$cat' and ja_subcat=''");
	...
	if(mysql_num_rows($subsel))
	{
		...
		{
			$responseText .= '<option value='.$result1['id'].'>'.stripslashes($result1['ja_subcat']).'</option>';
		}
	}
	else
		$responseText ='success#@#<option value="">Select Sub Category</option>';
...
else if($_REQUEST["query"]=="selectsubsearch")
{
	//$responseText="";
    $responseText="failure#@#";
	$cat=isset($_REQUEST["dublicate"])?$_REQUEST["dublicate"]:"";
	...
	$sql="select scat.id,scat.ja_subcat,scat.eurl from subcategory as scat, category as cat where scat.catid=cat.id and scat.status='Y' and cat.eurl='$cat'";
	$subsel = mysql_query($sql) or die(mysql_error());
	//$catres = mysql_fetch_assoc($subsel);
	if(mysql_num_rows($subsel))
	{
		$responseText ='success#@#<select name="selectsub" id="selectsub" class="inp6"><option value="All-Subcategory">All Subcategory</option>';
		while($result1 = mysql_fetch_assoc($subsel))
		{
			$responseText .= '<option value='.$result1['eurl'].'>'.stripslashes($result1['ja_subcat']).'</option>';
		}
		$responseText .= '</select>';
	}
	else { $responseText ='success#@#<select name="selectsub" id="selectsub" class="inp6"><option value="All-Subcategory">All Subcategory</option></select>'; }
	echo $responseText;
}
