...
if($query=="addtocart" || $query=="removefromcart" || $query=="updatecart" || $query=="viewcart" )
{
	...
	if($query=="addtocart")
	{
		$selpro = mysql_query("select id,jatitle,price from product where id ='$product_id'");
		$prores = mysql_fetch_assoc($selpro);
		$sql="insert into cart (productid,productname,price,qty,session_id) values('".$prores['id']."','".mysql_real_escape_string($prores['jatitle'])."','".$prores['price']."','1','".session_id()."')";
...
<table width="600" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	$product_id=$_REQUEST["product_id"];	
		...
		header("location:view-cart.html");
		exit;		
	}
