...
<div class="textfelid-searchbox">
      <label>
      <input name="textfield" type="text" id="textfield" value="<?php echo urldecode($_REQUEST['textfield']); ?>" onkeyup="return isEnter(event,'<?php echo $abspath; ?>')" class="inp5" />
      </label>
  </div>
...
$rscat1 = rs_select("category where status ='Y' and catname !='' ","*");
	while($rescat1 = mysql_fetch_assoc($rscat1))
	{
		...
		if($rescat1['eurl']==$_REQUEST['selectcat'])
		{
			$rssub1 = rs_select("subcategory where catid='".$rescat1['id']."' and status ='Y' and subcat !='' ","*");
